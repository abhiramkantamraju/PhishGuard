...
from phishguard import DANGEROUS_THRESHOLD, analyze_text, get_risk_level
from phishguard.validation import (
    EMPTY_INPUT_MESSAGE,
    MAX_EMAIL_LENGTH,
    validate_email_input,
)

from conftest import post_scan, scan_id_from_redirect


@pytest.fixture
def workflow_client(client, db_path):
    """
    A Flask test client backed by a throwaway database.

    The `client` fixture in conftest.py points app.config["DATABASE"] at
    pytest's tmp_path, so these tests never read or write the real
    phishguard.db, and disables CSRF and rate limiting — neither is what these
    tests are checking, and both are covered separately in test_app.py.
    """
...
def submit_scan(client, email_text):
    ...
    return post_scan(client, email_text)
...
def test_valid_phishing_email_is_scored_and_explained():
    ...
    than a domain. Each is a separate category, so together they must clear
    the Dangerous threshold rather than merely looking suspicious.
    """
    email = (
    ...
    assert score >= DANGEROUS_THRESHOLD
...
def test_valid_scan_is_saved_with_its_verdict(workflow_client):
    """
    ...
    """
    test_client, db_path = workflow_client
    email = (
        "URGENT: your account has been suspended. "
        "Verify your password now at http://192.168.10.44/login"
    )
    ...
    # The route redirects to the saved scan's permalink (post-redirect-get), so
    # a successful save is a 302 whose Location names the new row.
    assert response.status_code == 302
    assert scan_id_from_redirect(response) == 1
...
def test_rejected_scan_is_not_saved(workflow_client):
    """
    ...
    # Rejected input re-renders the form with the error rather than redirecting,
    # and answers 400 so the failure is visible to anything but a browser too.
    assert response.status_code == 400
    assert "Please paste an email message" in response.text
    assert rows_in_scans(db_path) == []


def test_each_valid_scan_is_saved_as_its_own_record(workflow_client):
    """
